...
for(yearSelect in c(2011:2010)){
  ...
  #datesForThisYear = unique(rawDat[,2])
 ...
 # }
  ...
  datesForThisYear = availableDates
	...
	availableDates = as.character(rev(availableDates))  #prioritize doing most recent...
    ...
    if(length(pickDates)<1) next
	  ...
	  eligible = by(dateRestrictedDat,dateRestrictedDat[,1],function(x) {
	  	if(length(grep("TMAX|TMIN|PRCP",x[,3]))==3){
	  		c(x[1,1],round(x[grep("TMAX",x[,3]),4]),round(x[grep("TMIN",x[,3]),4]),round(x[grep("PRCP",x[,3]),4]))
	  	}else{
	  		F
	  		} 
	  })
    if(all(unlist(lapply(eligible,function(x) x))==F)) next
	 ...
	 for(metric in colnames(fullStationDataFrame@data)){
		...
		writeRaster(testRaster,paste("Interpolation_Store/",eachDate,"_",metric,sep=""),overwrite=T,format="GTiff",datatype="INT2S")
		...
		#in rea life, remove the next and extract the points necessary here in a csv
		 ...
		 saveData[[length(saveData)+1]] = theseValues#be sure to divide values by 10 here!!!!
